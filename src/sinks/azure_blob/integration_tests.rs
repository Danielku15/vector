...
// Tags and metadata are applied when an append blob is first created and must persist across
// subsequent appends to the same blob.
async fn assert_append_blob_tags_and_metadata(config: AzureBlobSinkConfig) {
    let blob_prefix = format!("append-tags-meta/into/blob/{}", random_string(10));
    ...
    // Two separate batches: the first creates the blob (applying tags/metadata), the second
    // appends to it. Tags/metadata must still be present after the append.
    config
        .run_assert(random_lines_with_stream(100, 10, None).1)
        .await;
    config
        .run_assert(random_lines_with_stream(100, 10, None).1)
        .await;
    ...
    assert_eq!(
        blobs.len(),
        1,
        "both batches must target the same append blob"
    );
...
#[tokio::test]
async fn azure_blob_append_blob_tags_and_metadata() {
    assert_append_blob_tags_and_metadata(AzureBlobSinkConfig::new_emulator().await).await;
}

#[tokio::test]
async fn azure_blob_append_blob_tags_and_metadata_with_oauth() {
    assert_append_blob_tags_and_metadata(AzureBlobSinkConfig::new_emulator_with_oauth().await)
        .await;
}
